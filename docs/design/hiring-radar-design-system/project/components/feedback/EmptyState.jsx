...
const ALIAS = { radar: 'empty-radar', bot: 'agent-orb-idle', 'agent-orb': 'agent-orb-active' };
 ...
 */
export function HRIllustration({ name = 'empty-radar', size = 120, style, ...rest }) {
  const key = ALIAS[name] || name;
  const a = ART[key] || ART['empty-radar'];
  const rawUid = React.useId();
  const uid = rawUid.replace(/[^a-zA-Z0-9]/g, '');
  const inner = a.inner
    .replace(/id="([^"]+)"/g, (m, p) => 'id="' + p + '-' + uid + '"')
    .replace(/url\(#([^)]+)\)/g, (m, p) => 'url(#' + p + '-' + uid + ')');
 ...
 */
export function EmptyState({ illustration = 'empty-radar', title, description, action, onAction, actionIcon, style, ...rest }) {
  return (
    <div
      style={{
        display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center',
        gap: 6, padding: '40px 24px', maxWidth: 380, margin: '0 auto', ...style,
      }}
      {...rest}
    >
      <div style={{ marginBottom: 14, opacity: 0.92 }}>
        {typeof illustration === 'string' ? <HRIllustration name={illustration} size={120} /> : illustration}
      </div>
      {title && <h3 style={{ font: 'var(--text-h3)', color: 'var(--text-hi)' }}>{title}</h3>}
      {description && <p style={{ margin: 0, font: 'var(--text-sm)', color: 'var(--text-body)' }}>{description}</p>}
      {action && (
        <div style={{ marginTop: 14 }}>
          <Button variant="primary" iconLeft={actionIcon} onClick={onAction}>{action}</Button>
        </div>
      )}
    </div>
  );
